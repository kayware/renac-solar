...
renac_solar_ns = cg.esphome_ns.namespace("renac_solar")
RenacSolar = renac_solar_ns.class_(
    "RenacSolar", cg.PollingComponent, modbus.ModbusDevice
)
...
CONFIG_SCHEMA = (
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(RenacSolar),
...
async def to_code(config):
    ...
    if CONF_AMBIENT_TEMP in config:
        sens = await sensor.new_sensor(config[CONF_AMBIENT_TEMP])
    ...
    for i, pv in enumerate([CONF_PV1, CONF_PV2]):
        ...
        for sensor_type in pv_config:
            if sensor_type in pv_config:
                ...
                cg.add(getattr(var, f"set_string_{sensor_type}_sensor")(i, sens))